...
int main(){ 
    ...
    double total_t; 
                                            //To verify answer we will use 4 buffers 
    ...
    start_t = clock();              //Clocks at start of PING-PONG 
    if(myrank == 0){ 
    ...
    MPI_Barrier(MPI_COMM_WORLD);        //To SYNC 
    end_t = clock();//Clocks at end of PING-PONG

    if(myrank==0){ 
    total_t = (double)(end_t - start_t) / CLOCKS_PER_SEC;       //Convert into seconds 
